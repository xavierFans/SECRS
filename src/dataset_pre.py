...
class CRSDataset(Dataset):
    def __init__(
        ...
        data_file = os.path.join(dataset_dir, f'{split}_data_processed_user.jsonl')
        self.data = []
    ...
    def prepare_data(self, data_file):
        # 读取user_embeds
        # user_embeds = torch.load('./data/redial/KG_embs_best.pt')
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            # 先处理历史上文
            # context="user:utt<EOS>,system:utt<EOS>"
            # prompt_context="user:utt<SEP>,system:utt<SEP>"
            for line in tqdm(lines):
                ...
                for rec in dialog['rec']:
                    ...
                    data = {
                        ...
                        'rec': rec,
                        # 'user':user_embeds[dialog['userId']]
                    }
...
class CRSDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        for data in data_batch:
            ...
            # user_batch.append(data['user'])
            # userId_batch.append(data['userId'])
            # userEmbeds_batch.append(data['user_table'])
        input_batch = {}
        # context_batch={'input_ids':context_ids,'rec_labels':rec}
        ...
        for k, v in context_batch.items():
            if not isinstance(v, torch.Tensor):
                ...
                # print(f"context_batch[{k}] size :"+str(context_batch[k].size()))
        ...
        for k, v in prompt_batch.items():
            if not isinstance(v, torch.Tensor):
                ...
                # print(f"prompt_batch[{k}] size :"+str(prompt_batch[k].size()))
                # print(f"prompt_batch[{k}] :"+str(prompt_batch[k]))
        ...
        # user_batch=(torch.stack(user_batch, dim=1)).to(self.device)
        # userId_batch=(torch.stack(userId_batch, dim=0)).to(self.device)
        # userId=data['userId'].to(self.device)
        # userEmbeds_batch = (torch.stack(userEmbeds_batch, dim=1)).to(self.device)
        # input_batch['user']=user_batch
        # input_batch['userId']=userId
        # input_batch['userEmbeds']=userEmbeds_batch
        # input_batch={'context':[context_batch],'prompt':[prompt_batch],'entity':entity_batch}
        return input_batch
