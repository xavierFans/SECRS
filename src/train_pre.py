...
from dataset_entityKG import EntityKG
...
"""
    train_pre.py: Prompt Pre-training
"""
...
if __name__ == '__main__':
    ...
    entity_kg = EntityKG(dataset=args.dataset, debug=args.debug).get_entity_kg_info()
    
    # tokenizer = AutoTokenizer.from_pretrained(args.tokenizer)
    tokenizer = AutoTokenizer.from_pretrained("../DialoGPT-small") 
    tokenizer.add_special_tokens(gpt2_special_tokens_dict)

    model = PromptGPT2forCRS.from_pretrained("../DialoGPT-small")
    ...
    prompt_encoder = KGPrompt(
        ...
        entity_kg_n_entity=entity_kg['num_entities'],entity_kg_num_relations=entity_kg['num_relations'],
        entity_kg_edge_index=entity_kg['edge_index'], entity_kg_edge_type=entity_kg['edge_type']
    ).to(device)

    # print("text_encoder.config.hidden_size:"+str(text_encoder.config.hidden_size))
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if valid_report[f'valid/{metric}'] * mode > best_metric * mode:
            best_epoch = epoch
        prompt_encoder.save_epoch(best_metric_dir,epoch)
        best_metric = valid_report[f'valid/{metric}']
        logger.info(f'new best model with {metric}')
    ...
    prompt_encoder.save(final_dir,best_epoch)
