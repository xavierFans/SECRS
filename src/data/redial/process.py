    ...
    with open(data_file, 'r', encoding='utf-8') as fin, open(out_file, 'w', encoding='utf-8') as fout:
        for line in tqdm(fin):
            ...
            while turn_i < len(messages):
                ...
                utt_turn = [] # 存储每轮对话语句
                entity_turn = [] # 存储每轮对话语句中提到的entity id
                movie_turn = [] # 存储每轮对话语句中提到的movie id
...
if __name__ == '__main__':
    ...
    process('valid_data_dbpedia.jsonl', 'valid_data_processed_user.jsonl', item_set)
    process('test_data_dbpedia.jsonl', 'test_data_processed_user.jsonl', item_set)
    process('train_data_dbpedia.jsonl', 'train_data_processed_user.jsonl', item_set)
