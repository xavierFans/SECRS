...
from utils import FocalLoss,MultiClassFocalLoss,LSLoss
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if prompt_embeds is not None:
                prompt_attention_mask = prompt_embeds.new_ones((batch_size, prompt_embeds.shape[-2])) # (batch_size,prompt_len)
...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if rec:
            ...
            rec_logits @= entity_embeds.T  # (bs, n_item) tensor

            if rec_labels is not None:
                # rec_loss = F.cross_entropy(rec_logits, rec_labels)

                rec_loss = self.LSloss(rec_logits,rec_labels) 
