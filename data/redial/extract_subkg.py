...
"""
    提取知识图谱DBpedia，生成每个结点只进行两跳的知识图谱子图，再将KG的entity、relation全部转为id
"""
...
def extract_subkg(kg, seed_set, n_hop):
    ...
    采用RippleNet提取知识图谱的hrt：
        seed：head_entity 激起水花的石头
        ripple_set：由seed通过n跳得到的涟漪实体

    Args:
        kg (dict): {head entity: [(relation, tail entity)]}
        seed_set (list or set): [entity]
        n_hop (int): 通过长度为n的边查询实体
...
def kg2id(kg):
    ...
    # 将entity、relation、kg中的entity、relation转化为id
